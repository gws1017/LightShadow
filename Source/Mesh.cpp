...
#ifdef _WITH_TERRAIN_TESSELATION
	int nIncrease = 4; //(Block Size == 9) ? 2, (Block Size == 13) ? 3  (Block Size == 17) ? 4
	//13 / 4(점5개라 4등분) = 3, 17 / 4 = 4
	//블록 너비와함께 바꿔주어야한다.
...
CSphereMeshIlluminated::CSphereMeshIlluminated(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fRadius, UINT nSlices, UINT nStacks) : CMeshIlluminated(pd3dDevice, pd3dCommandList)
{
	...
	k = 0;
	//왼쪽 부분(m_nIndices) (nSlices * 3) ____ 1
	for (UINT i = 0; i < nSlices; i++)
	{
		pnIndices[k++] = 0;
	...
	//왼쪽 부분(m_nIndices) (nSlices * 3) ____ 2
	for (UINT i = 0; i < nSlices; i++)
	{
	...
	CalculateVertexNormals(pxmf3Normals, pxmf3Positions, m_nVertices, pnIndices, m_nIndices); //정점기반으로 노멀계산

	//생성해둔 정점들을 조명 정점으로 변환
...
CPlaneMeshIlluminated::CPlaneMeshIlluminated(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList, float fWidth, float fHeight, float fDepth, float fxPosition, float fyPosition, float fzPosition)
