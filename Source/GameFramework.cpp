...
void CGameFramework::CreateDirect3DDevice()
{
	...
	::gnRtvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV); //stdafx.h
	::gnDsvDescriptorIncrementSize = m_pd3dDevice->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_DSV); //stdafx.h
...
void CGameFramework::CreateRenderTargetViews()
{
	...
	for (UINT i = 0; i < m_nSwapChainBuffers; i++)
	{
		...
		d3dRtvCPUDescriptorHandle.ptr += ::gnRtvDescriptorIncrementSize;//stdafx.h
	}
}
...
{
	//WaitForGpuComplete();
	::WaitForGpuComplete(m_pd3dCommandQueue, m_pd3dFence, ++m_nFenceValues[m_nSwapChainBufferIndex], m_hFenceEvent); //stdafx.h
		...
		case WM_KEYUP:
			switch (wParam)
			{
				...
				case 'D':
					...
					break;
				case 'W':
					TerrainTessellationWire = !TerrainTessellationWire;
					break;
				case'I':
					Teleport = !Teleport;
					break;
...
void CGameFramework::BuildObjects()
{
	...
	m_pPlayer = new CTerrainPlayer(m_pd3dDevice, m_pd3dCommandList, m_pScene->GetGraphicsRootSignature(), m_pScene->GetTerrain(), m_pScene->m_ppBullets, 1);
	m_pCamera = m_pPlayer->GetCamera();
	m_pScene->m_pPlayer = m_pPlayer;
	m_pScene->SetPlayer();

	m_pScene->SetParticlePos();

	CreateShaderVariables();

	WaitForGpuComplete(m_pd3dCommandQueue, m_pd3dFence, ++m_nFenceValues[m_nSwapChainBufferIndex], m_hFenceEvent);
	::ExecuteCommandList(m_pd3dCommandList, m_pd3dCommandQueue, m_pd3dFence, ++m_nFenceValues[m_nSwapChainBufferIndex], m_hFenceEvent); //stdaf.cpp
...
{    
	...
	m_pScene->OnPreRender(m_pd3dCommandList,m_pd3dDevice, m_pd3dCommandQueue, m_pd3dFence, m_hFenceEvent);
